...
__global__ void skyline(const float *P, int *s, int D, int N) {
    const int i = blockIdx.y * blockDim.y + threadIdx.y;
    const int j = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < N && j < N && dominates(&P[i * D], &P[j * D], D)) {
        s[j] = 0;
    }
}
...
int skyline(const points_t *points, int *s) {
    ...
    const float *d_P;
    int *d_s;
    const size_t SIZE_P = N * D * sizeof(*d_P);
    const size_t SIZE_s = N * sizeof(*d_s);

    cudaSafeCall(cudaMalloc((void **)&d_P, SIZE_P));
    cudaSafeCall(cudaMalloc((void **)&d_s, SIZE_s));
    cudaSafeCall(cudaMemcpy((void *)d_P, P, SIZE_P, cudaMemcpyHostToDevice));
    cudaSafeCall(cudaMemcpy((void *)d_s, s, SIZE_s, cudaMemcpyHostToDevice));
    ...
    cudaSafeCall(cudaMemcpy((void *)s, (void *)d_s, SIZE_s, cudaMemcpyDeviceToHost));
    cudaSafeCall(cudaFree((void *)d_P));
    cudaSafeCall(cudaFree((void *)d_s));
